"""I/O helpers, including an xlsx fallback reader for simple Excel sheets."""
...
def read_table(path: Optional[str], sheet_name: Optional[str | int] = 0) -> Optional[pd.DataFrame]:
    ...
    if suffix in {".xlsx", ".xlsm"}:
        try:
            ...
            # The clinical metadata is a simple worksheet, so a zip/xml fallback is sufficient here.
            return read_xlsx_fallback(path_obj, sheet_name=sheet_name)
    if suffix == ".xls":
        raise ValueError("Legacy .xls files are not supported without an Excel engine.")
