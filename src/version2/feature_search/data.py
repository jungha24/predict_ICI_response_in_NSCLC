...
def infer_feature_family(feature_name: str) -> str:
    ...
    if "__" in feature:
        stem, rest = feature.split("__", 1)
        if rest.startswith("Centroid_"):
            return f"centroid::{stem}"
        if rest.startswith("curated_gene_"):
            return f"signature::{stem}"
        if rest.startswith("cellrank_"):
            return f"cellrank::{stem}"
        return f"file::{stem}"
    if feature.startswith("Centroid_"):
        return "centroid"
    if feature.startswith("curated_gene_"):
        return "signature"
    if feature.startswith("cellrank_"):
        return "cellrank"
