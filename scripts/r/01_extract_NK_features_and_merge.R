...
NK_subset<- subset(seu_integrated, idents = c('NK'))
DefaultAssay(NK_subset) <- 'RNA'
NK_subset$source_obj <- ifelse(grepl("^DIS_", colnames(NK_subset)), "disease", "normal")
NK_list <- SplitObject(NK_subset, split.by = "source_obj")
names(NK_list)

NK_list <- lapply(NK_list, function(x) {
...
subset <- FindNeighbors(subset, dims = 1:6) #<<< 
subset <- FindClusters(subset, resolution = 0.1) #<<<
subset <- RunUMAP(subset, dims = 1:6) #<<<
DimPlot(subset, reduction='umap')#3x4
DimPlot(subset(subset,predicted.celltype.l2 %in% c('NK','NK Proliferating','NK_CD56bring')), reduction='umap', group.by = 'predicted.celltype.l3',label=T)#3x4
...
markers %>%
  ...
  slice_head(n = 10) %>%
  ungroup() -> top10
subset$sample <- ifelse(is.na(subset$sample),subset$donor_id,subset$sample)
...
plot_df <- meta_df %>%
  ...
  count(cohort_l2_resp, sample, seurat_clusters, name = "n") %>%
...
ggplot(plot_df2, aes(x = sample_plot, y = prop, fill = seurat_clusters)) +
  geom_col(width = 0.9) +
  ...
  )#4x10 #20260327_NK_subcluster_change

subset$manual.cluster_l2 <- ifelse(subset$seurat_clusters==0,'immediate-early NK',
                                    ifelse(subset$seurat_clusters==1, 'resting NK',
                                           ifelse(subset$seurat_clusters==2,'NKT/gdT contam',
                                                 'proliferating NK')))
ggplot(subset@meta.data[subset@meta.data$cohort_l2=='Lee_p1_base',],aes(x=sample, fill=manual.cluster_l2))+geom_bar(position='fill')
DimPlot(subset, reduction='umap',group.by='manual.cluster_l2', label=T)
VlnPlot(subset, features=c('NCAM1','SELL','CCR7','GZMK','XCL1','XCL2','KLRC1','KLRB1','LTB','TCF7'),pt.size=0)
FeaturePlot(subset, features=c('NCAM1','SELL','XCL1','XCL2','KLRC1','KLRB1'),order=T)#6x5 #20260327_CD56bright_NK_featureplot
VlnPlot(subset, features=c("NR4A2", "FOS", "FOSB", "DUSP1", "TNFAIP3", "BTG2", "PPP1R15A"),pt.size=0)
FeaturePlot(subset, features=c('FCGR3A','FGFBP2','PRF1','GNLY','GZMB','GZMH','CX3CR1','SPON2','ADGRG1','KLRD1','FCER1G','TYROBP'),order=T)#8x10#20260327_cytotoxic_NK_featureplot
VlnPlot(subset, features=c('CCL5','GZMH','XCL2','FCER1G','ADGRG1'),pt.size=0)


EARLY_RESPONSE_MARKER = ["NR4A2", "FOS", "FOSB", "DUSP1", "TNFAIP3", "BTG2", "PPP1R15A"]
CD56_MARKER = ['GZMK','TCF7','IGFBP4','CCR7','AREG','SELL','XCL1','XCL2','IL7R']
RESTING_MARKER = ['FCER1G','KLRC1','KLRB1','IL12RB2','TMIGD2','AKR1C3']
PROLIFERATING_MARKER =['STMN1','TYMS','MCM7','PCLAF','MKI67','PCNA','MCM5','LMNB1','NASP']



######## immediate-eaerly exclusion ##########
subset_subset<- subset(subset, manual.cluster_l2 != c('immediate-early NK'))
DefaultAssay(subset_subset) <- 'RNA'
subset_subset_list <- SplitObject(subset_subset, split.by = "source_obj")
...
subset_subset <- IntegrateData(
  anchorset = anchors,
  dims = 1:30 #<<<
)

subset_subset <- ScaleData(subset_subset, verbose = FALSE)
subset_subset <- RunPCA(subset_subset, npcs = 50, verbose = TRUE)
DimPlot(subset_subset, reduction='pca', group.by='predicted.celltype.l2')
DimPlot(subset_subset, reduction='pca', group.by='manual.cluster_l2')
ElbowPlot(subset_subset) #3x4 #20260324_B_subcluster_PCA_elbowplot
DimHeatmap(subset_subset, dims = 1:10, cells = 500, balanced = TRUE) #8x8 #20260324_B_subcluster_PCA_pcaheatmap
subset_subset <- FindNeighbors(subset_subset, dims = 1:6) #<<< 
subset_subset <- FindClusters(subset_subset, resolution = 0.1) #<<<
subset_subset <- RunUMAP(subset_subset, dims = 1:6) #<<<
DimPlot(subset_subset, reduction='umap',group.by='manual.cluster_l3',label=T)#4x6 #20260327_NK_exclude_IER_subcluster

subset_subset$manual.cluster_l3 <- ifelse(subset_subset$seurat_clusters==0,'resting NK',
                                   ifelse(subset_subset$seurat_clusters==1, 'NKT contamination',
                                          ifelse(subset$seurat_clusters==2,'ambiguous',
                                                 ifelse(subset_subset$seurat_clusters==3,'CD56bright NK','proliferating NK'))))


cells_all <- colnames(subset)
manual_l3_new <- setNames(
  rep("Immediate-early response", length(cells_all)),
  cells_all
)
## subset_subset에 있는 세포의 manual.cluster_l3 값으로 덮어쓰기
cells_sub <- intersect(colnames(subset_subset), cells_all)
manual_l3_new[cells_sub] <- as.character(
  subset_subset@meta.data[cells_sub, "manual.cluster_l3", drop = TRUE]
)
## subset object에 새 column으로 저장
subset$manual.cluster_l3 <- manual_l3_new[colnames(subset)]

table(subset$manual.cluster_l3, useNA = "ifany")
DimPlot(subset, group.by = 'manual.cluster_l3',label=T) #4x6 #20260327_NK_level3_umap
DimPlot(subset(subset, manual.cluster_l3 %in% c('CD56bright NK','Immediate-early response','proliferating NK','resting NK')), group.by = 'manual.cluster_l3',label=T) #4x6 #20260327_NK_clean_level3_umap

saveRDS(subset, file='data/20260309_pilot/results/version2/NK/NK_subset.rds')

#subset_clean <- subset(subset, manual.cluster_l3 %in% c('CD56bright NK','Immediate-early response','proliferating NK','resting NK'))

DimPlot(subset, reduction='umap',group.by='manual.cluster_l2') # 4x6 #20260327_NK_level2_umap

cell_df <- subset_clean@meta.data[subset_clean@meta.data$time =='base' & subset_clean@meta.data$cohort =='Lee_p1',] %>%
  dplyr::select(sample, manual.cluster_l3)
count_df <- subset_clean@meta.data[subset_clean@meta.data$time =='base' & subset_clean@meta.data$cohort =='Lee_p1',] %>%
  count(sample, manual.cluster_l3, name = "n_cells")
total_df <- subset_clean@meta.data[subset_clean@meta.data$time =='base' & subset_clean@meta.data$cohort =='Lee_p1',] %>%
  count(sample, name = "total_cells")
prop_df <- count_df %>%
  left_join(total_df, by = "sample") %>%
  mutate(prop = n_cells / total_cells)

ggplot(prop_df,aes(x=n_cells))+geom_histogram(bins=70)+facet_wrap(~manual.cluster_l3, scales='free')
ggplot(prop_df,aes(x='subcluster',y=n_cells))+geom_boxplot(outlier.size=0)+geom_beeswarm(alpha=0.5, cex=3)+facet_wrap(~manual.cluster_l3, scales='free')

############
subset_clean <- subset(subset, subset = manual.cluster_l2 %in% c('immediate-early NK','resting NK'))
subset_clean <- subset(subset, subset = manual.cluster_l3 %in% c('Immediate-early response','resting NK','CD56bright NK'))
...
DimHeatmap(subset_clean, dims = 1:10, cells = 500, balanced = TRUE) #8x8 #20260328_NK_clean_PCA
###
...
# 0. USER CONFIG
# =========================================================
TARGET_ANALYSIS <- "NK"   # "Monocyte" or "NK"
...
ANCHOR_STATE_LOW  <- 'resting NK'
ANCHOR_STATE_HIGH <- 'immediate-early NK'
CENTROID_PREFIX   <- if (TARGET_ANALYSIS == "NK") "mono_low_to_high" else "nk_low_to_high"
...
CELLRANK_PATIENT_CSV <- "data/20260309_pilot/results/version2/NK/CellRank_output_from_clean_subset/cellrank_b3_patient_features.csv"       # if provided and exists, merge CellRank patient features
...
if (!is.na(ANCHOR_STATE_LOW) && !is.na(ANCHOR_STATE_HIGH)) {
  centroid_res <- calc_two_anchor_score(
    ...
    cohort_col = "cohort_l2",
    target_cohort = "Lee_p1_base",
    low_label = "resting NK",
    high_label = "immediate-early NK",
    assay_use = "RNA",
    rerun_pca = TRUE,
    dims_use = 1:3,
    npcs = 20,
...
final_feature_df_cellrank_filt <- final_feature_df_cellrank[,!(colnames(final_feature_df_cellrank) %in% c('denovo_gene_mp1__n_overlap','denovo_gene_mp2__n_overlap', 'denovo_gene_mp3__n_overlap', 'denovo_gene_mp4__n_overlap', 'denovo_gene_mp5__n_overlap', 'denovo_gene_mp6__n_overlap', "denovo_gene_mp1__singscore" ,"denovo_gene_mp4__singscore" ,"denovo_gene_mp5__singscore" , 'Centroid_n_score_cells',  'Centroid_keep_for_analysis', 'cellrank_cr_b3_priming_mean', 'cellrank_cr_b3_priming_q75', 'cellrank_cr_b3_n_cells', 'cellrank_qc_b3_subset_frac_CD56bright NK','cellrank_qc_b3_subset_frac_Immediate-early response','cellrank_qc_b3_subset_frac_resting NK'))]
OUT_CSV <- "data/20260309_pilot/results/version2/NK/CellRank_output_from_clean_subset/final_feature_df_with_cellrank_b3_filt.csv"
